...
class ft_net_VGG16(nn.Module):
    ...
    def forward(self, x):
        ...
        # --------------------------------------------------------------------
        mask = hfs(x=x, n=3, s=2, use_square_kernel=True)
        x = mask * x  # Superimposed mask3
        # --------------------------------------------------------------------
...
class ft_net_cvusa_LPN(nn.Module):
    ...
    def forward(self, x):
        ...
        #######**********************  Insert  HFS *********************#######
        mask3 = hfs(x=x, n=3, s=2, use_square_kernel=True)
        x = mask3 * x  # Superimposed mask2 and 3
        # ----------------------------------------------------------------------
        x = self.model.layer4(x)
        #######**********************  Insert  HFS *********************#######
        mask4 = hfs(x=x, n=3, s=2, use_square_kernel=True)
        x = (mask4 + mask3) * x  # Superimposed mask2 and 3
        # ----------------------------------------------------------------------
        # print(x.size())
        if self.pool == 'avg+max':
            x1 = self.avgpool(x)
...
# Test the model, before you train it. 
    net = three_view_net(701, droprate=0.5, pool='avg', stride=1, VGG16=False, LPN=True, block=8)
    ...
    macs, params = profile(net, inputs=(input, input, input))
